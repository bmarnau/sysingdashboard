...
text = regex_once(
    ...
    r'(roadmap:\n.*?)  - id: "BSF-03"\n    title: "Kundenverantwortung und Kundensicht"\n.*?(?=  - id: "BSF-03D"\n)',
    r'\1',
    "remove completed sprint from roadmap",
)
...
text = once(text, 'nextReleaseTarget: "BSF-03 P5 Kundenverantwortung verwalten"', 'nextReleaseTarget: "BSF-03D Arbeitspaket-Kategorien"', "next release")
write(path, text)

# CURRENT STATUS
path = "docs/CURRENT-STATUS.md"
text = read(path)
text = once(text, "- Dashboard-Version: `1.60.0`", "- Dashboard-Version: `1.61.0`", "status version")
...
text = once(text, "2. **BSF-03 — IN ARBEIT; P5 NÄCHSTER SCHRITT**\n3. **BSF-03D — GEPLANT / bis BSF-03-Abschluss gesperrt**", "2. **BSF-03 — DONE**\n3. **BSF-03D — NÄCHSTER SCHRITT / FREIGEGEBEN**", "priorities order")
write(path, text)
...
text = once(text, "`BSF-03/P5 → BSF-03-Abschluss → BSF-03D → BSF-03A → BSF-03B → BSF-03E → BSF-03C → BSF-DOC-01 → BSF-DOC-02 → BSF-DOC-03 → BSF-04 → BSF-04A → BSF-05 → BSF-06 → BSF-07 → BSF-09 → BSF-10 → BSF-FINAL → INTEGRATION-READINESS`", "`BSF-03D → BSF-03A → BSF-03B → BSF-03E → BSF-03C → BSF-DOC-01 → BSF-DOC-02 → BSF-DOC-03 → BSF-04 → BSF-04A → BSF-05 → BSF-06 → BSF-07 → BSF-09 → BSF-10 → BSF-FINAL → INTEGRATION-READINESS`", "sprint sequence")
...
text = once(text, "| Aktueller Stand     | Version 1.60.0; MVP-Baseline CLOSED/PASS; BSF-03 P3/P4 „Meine Kunden“ read-only umgesetzt; P5 Verantwortungsverwaltung offen.", "| Aktueller Stand     | Version 1.61.0; MVP-Baseline CLOSED/PASS; BSF-03 Kundenverantwortung und Kundensicht P1–P5 abgeschlossen; BSF-03D folgt.", "diary row")
