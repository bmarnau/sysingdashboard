/**
 * Erzeugt die verteilbaren Fassungen von SYSING-KIOSK-001 aus **einer** Quelle:
 * `docs/SYSING-KIOSK-001_*.md`. Ausgabe ist eine Word-Datei (docx). Die PDF-Fassung
...
function sourceFile() {
  const match = readdirSync(DOCS_DIR)
    .filter((f) => /^SYSING-KIOSK-001_.*\.md$/.test(f))
    .sort();
  if (match.length !== 1) {
    throw new Error(`Erwartet genau eine SYSING-KIOSK-001-Quelle, gefunden: ${match.length}`);
  }
...
async function main() {
  ...
  const doc = new Document({
    ...
    title: `${documentId} — Info-Kiosk-Datenschnittstelle`,
  ...
  const name = `SYSING-KIOSK-001_Info-Kiosk-Datenschnittstelle_V${version}.docx`;
