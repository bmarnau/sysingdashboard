...
function sameSet(left, right) {
  ...
  return a.length === left.length && b.length === right.length && a.every((value, i) => value === b[i]);
}
...
assert(tdfSources.length === 1, `Erwartet genau eine SYSING-KIOSK-001-Quelle, gefunden: ${tdfSources.length}`);
...
assert(metadata(document, "document_id") === "SYSING-KIOSK-001", "document_id muss SYSING-KIOSK-001 sein.");
...
assert(metadata(document, "source_review_date") === "2026-09-23", "source_review_date muss 2026-09-23 sein.");
assert(metadata(document, "referenzdatensatz") === `\`${referencePath}\``, "referenzdatensatz muss auf die führende JSON-Datei zeigen.");
assert(metadata(document, "demo_schema") === `\`${schemaVersion}\``, "demo_schema im Dokument muss dem Codevertrag entsprechen.");
...
assert(codeDomains.length === 6, `KIOSK_DOMAIN_IDS muss sechs Einträge enthalten, gefunden: ${codeDomains.length}`);

assert(reference.schemaVersion === schemaVersion, "Referenz-JSON schemaVersion driftet vom Codevertrag.");
...
assert(reference?.snapshot?.datasetVersion === datasetVersion, "Referenz-JSON datasetVersion driftet vom Codevertrag.");
assert(sameSet(referenceDomains, codeDomains), "Referenz-JSON Domain-IDs driften von KIOSK_DOMAIN_IDS.");

assert(document.includes(`\`${schemaVersion}\``), "Dokument nennt die implementierte Demo-Schema-Version nicht.");
assert(document.includes(`\`${datasetVersion}\``), "Dokument nennt die implementierte Dataset-Version nicht.");
assert(document.includes(`\`${referencePath}\``), "Dokument nennt die führende Referenzdatei nicht.");
assert(
