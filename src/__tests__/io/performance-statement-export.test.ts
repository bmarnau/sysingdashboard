...
describe("BSF-03B administrativer JSON-Gesamtdatenvertrag", () => {
  it("exportiert die interne Statement-Serie additiv in Schema 1.3.0", async () => {
    ...
    expect(result.document.performanceStatements?.statements[0].snapshotHash).toBe("1".repeat(64));
    expect(result.document.performanceStatements?.statements[1].snapshotHash).toBe("2".repeat(64));
