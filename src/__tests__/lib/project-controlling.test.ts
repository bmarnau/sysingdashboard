...
describe("BSF-03A provider-neutral project controlling", () => {
  ...
  it("summarizes the same filtered decimal-hour rows reproducibly regardless of repository order", async () => {
    ...
    const forward = await new ProjectControllingService(makeRepository(sourceRows)).get(BASE_FILTERS);
    const reverse = await new ProjectControllingService(makeRepository([...sourceRows].reverse())).get(
      BASE_FILTERS,
    );
