...
import { saveUsers, setActiveUserId, bootstrap, type UserProfile } from "@/lib/user-management";

afterEach(() => cleanup());

function seedAs(role: UserProfile["role"]): UserProfile {
  const now = "2026-06-01T00:00:00.000Z";
  const u: UserProfile = {
    id: `usr-${role}`, firstName: role, lastName: "X", displayName: role,
    email: `${role}@t.local`, phone: "", role, status: "active", mfaEnabled: false,
...
describe("Manipulation – localStorage-Tampering (offen dokumentiert)", () => {
  it("should_grantSysadminUiGate_when_localStorageForgesRole_KNOWN_GAP_SEC_CRIT_002", () => {
    // KEIN Backend-Check. Wer im Browser die Rolle setzt, sieht Sysadmin-UI.
  ...
  it("should_resetBootstrap_when_activeUserPointsToUnknownId", () => {
    // Bootstrap fällt auf einen vorhandenen Benutzer zurück, wenn die
    // aktive ID nicht existiert (kein Panic-Auth, keine Fake-Rolle).
    seedAs("viewer");
...
describe("Manipulation – Assignment-Injection (Import-Pipeline)", () => {
  ...
  it("should_notInterpretUserSuppliedAssignments_when_envelopeHasNoAssignmentsField", () => {
    // Das aktuelle Envelope kennt keinen `assignments`-Slot — ein Angreifer
    // kann daher keine RoleAssignments via JSON-Import injizieren.
    // Falls das Feld jemals ergänzt wird, MUSS dieser Test angepasst UND
    // ein Zod-Guard gegen `source: "entra"` bei lokalem Import ergänzt werden.
    const raw = {
      schemaVersion: "1.0.0",
      exportType: "full",
...
describe("Manipulation – Replay abgelaufener / manipulierter Assignments", () => {
  ...
  const base: RoleAssignment = {
    id: "a-1", principalId: anyUser.id, principalType: "user",
    role: "administrator", scope: SCOPE_ROOT, source: "local",
    grantedAt: "2026-01-01T00:00:00Z", grantedBy: "test",
  };
  ...
  it("should_denyAccess_when_scopePayloadMismatch", () => {
    ...
    expect(
      evaluateAccess(anyUser, "users.manage", {
        assignments: [wrongScope], scope: "tenant:home", now: REF_NOW,
      }),
    ).toBe(false);
  });

  it("should_notEscalate_when_clientTriesToRunCanWithForgedRole", () => {
    // `can()` liest die Rolle aus dem übergebenen UserProfile — kein
    // ambientes Escalation-Vehikel im Argument (also mindestens der
    // TS-Contract ist eng). Nach Auth-Einführung MUSS die Rolle
    // serverseitig verifiziert werden.
    const forged = { ...anyUser, role: "systemadministrator" as const };
    expect(can(forged, "roles.manage")).toBe(true); // Ist-Zustand: forged wins → siehe SEC-CRIT-002
  });
});
