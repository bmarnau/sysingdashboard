...
const baseReport = {
  schemaVersion: "2.0.0",
  identity: {
    dashboardVersion: "1.43.0",
    commit: "abc123",
    environment: { node: "v22", platform: "linux" },
  },
...
describe("technical-report canonical hash", () => {
  ...
  it("ändert sich bei relevantem Feldwechsel (Severity)", () => {
    ...
    const changed = {
      ...baseReport,
      findings: [{ ...baseReport.findings[0], severity: "HIGH" }, baseReport.findings[1]],
    };
  ...
  it("extractIntegrityPayload klammert flüchtige Felder aus", () => {
    const payload = extractIntegrityPayload({
      ...baseReport,
      generatedAt: "x",
      id: "y",
      version: 99,
...
describe("release-gate", () => {
  it("schlägt production vor bei sauberem Report", () => {
    const p = proposeReleaseStage({
      findings: [],
      sections: { auth: { status: "passed" } },
      sources: { backup: { status: "passed" }, docs: { status: "passed" } },
      blockers: [],
    });
  ...
  it("degradiert auf development bei offenem Critical", () => {
    const p = proposeReleaseStage({
      findings: [{ severity: "CRITICAL", accepted: false }],
      sections: {},
      sources: {},
      blockers: [],
    });
  ...
  it("degradiert auf internal-test bei HIGH", () => {
    const p = proposeReleaseStage({
      findings: [{ severity: "HIGH", accepted: false }],
      sections: { auth: { status: "passed" } },
  ...
  it("Override überschreibt effective, nicht proposed", () => {
    ...
    const applied = applyReleaseOverride(proposal, {
      stage: "pilot",
      by: "admin",
      reason: "r",
      ticket: "T-1",
  ...
  it("wirft bei unbekannter Override-Stufe", () => {
    expect(() =>
      applyReleaseOverride({ proposed: "pilot", reason: "" }, { stage: "invalid" }),
    ).toThrow();
  });
