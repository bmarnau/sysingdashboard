-- BSF-03A / Issue #106 — Project Controlling Permission + Kategorie-Bruecke
-- Additiv. Keine alte Migration wird geaendert.
--
-- Wichtige Security-Vertraege:
--   * public.has_permission bleibt SECURITY DEFINER und behaelt seine ACL.
--   * public.bsf02c_publish_shared_projection_snapshot bleibt SECURITY INVOKER.
--   * RLS/Grants der Shared-Projection-Tabellen werden nicht gelockert.
--   * Kein FK auf reference_value: unbekannte historische category_key bleiben stabil.

ALTER TABLE public.shared_work_package_projection
  ADD COLUMN IF NOT EXISTS category_key text NULL;

ALTER TABLE public.shared_work_package_projection
  ADD COLUMN IF NOT EXISTS category_observed boolean NOT NULL DEFAULT false;

CREATE INDEX IF NOT EXISTS shared_work_package_projection_active_category_idx
  ON public.shared_work_package_projection (systemhouse_id, customer_id, category_key)
  WHERE is_active;

-- ---------------------------------------------------------------------------
-- RBAC: neue atomare Permission project.controlling.view.
-- Bestehenden SECURITY-DEFINER-/ACL-Vertrag explizit erhalten.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- BSF-02C Publish-RPC: exakte Signatur bleibt bestehen; category_key wird
-- rueckwaertskompatibel verarbeitet. Alte Payloads ohne Schluessel veraendern
-- bei Updates die bereits beobachtete Kategorie nicht.
-- ---------------------------------------------------------------------------
...
-- ACL explizit erneut festschreiben (CREATE OR REPLACE behaelt sie zwar, aber
-- die Migration soll auch den Security-Vertrag selbst dokumentieren).
...
COMMENT ON FUNCTION public.bsf02c_publish_shared_projection_snapshot(
  uuid, uuid, text, boolean, jsonb, jsonb, jsonb, text[], text[], text[]
) IS 'BSF-02C/03A: atomarer Shared-Projection-Snapshot-Publish im User-JWT; SECURITY INVOKER, RLS bleibt aktiv, Kategorie rueckwaertskompatibel beobachtet.';
